...
```{r}
#| label: load-data
#download.file("https://jcanner.github.io/stat_210_2025_website/docs/labs/student/data/buoy_2024.txt", 
             # destfile = "buoy_2023.txt")
read_table("data-raw/buoy_2023.txt")
```
...
read_table("data-raw/buoy_2023.txt", comment = "#")
```
...
#| label: clean-names
library(janitor)
data <- read_table("data-raw/buoy_2023.txt", comment = "#")
clean_names(data)%>% 
  rename(mm = mm,
         mm_2 = mm_2)
```
...
write_csv(data, "data-raw/buoy_update.csv")
...
#| eval: false
data <- read_csv("data-raw/buoy_update.csv", 
...
library(viridis)
data <- read_csv("data-clean/buoy_update.csv")
...
ggplot(data, aes(x = MM,
  ...
  geom_smooth(se = FALSE, 
              color = "black",
              show.legend = FALSE) +
  labs(title = "Monterey Bay Canon Air Temperature in 2024",
...
Data on the kinds and frequency of police encounters with people, including arrests, are included in the Tidy Tuesday dataset for different areas.  It enables the investigation of police operations, demographic variables, and crime patterns.

Questions:
How many encounters with the police occurred in each area?
 Which kind of police encounter occurs most frequently?
 Which month saw the highest number of arrests?
